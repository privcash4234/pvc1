...
#if defined(NDEBUG)
# error "Privcash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x30c074dd7274a2f52ac9741ec090c0a2f41a70c9caf6f2e5b6aed3d6933265d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Privcash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Privcash Signed Message:\n";
...
{
    // Privcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Privcash: 1 day
static const int64 nTargetSpacing = 5 * 60; // Privcash: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Privcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Privcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x30c074dd7274a2f52ac9741ec090c0a2f41a70c9caf6f2e5b6aed3d6933265d4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "20/4/2018 privacy for everyone";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("987684710fa689ad5023690c86f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4ba0f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524233037;
        ...
        assert(block.hashMerkleRoot == uint256("0xb48f8b5258460876d3e9977ff621e80cad9776cf5828930543db9e08112117ef"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Privcash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrivcashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrivcashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PrivcashMiner(CWallet *pwallet)
{
    printf("PrivcashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("privcash-miner");
    ...
    try { loop {
        ...
        printf("Running PrivcashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PrivcashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PrivcashMiner, pwallet));
}
